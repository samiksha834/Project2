...
def callAdmin():
    while True:
        ...
        if ch == 1:
            ...
            mycursor.execute(
                "insert into user_profile(username,password,name,email,role) values ('{}', '{}', '{}', '{}','student')".format(
                    un, pwd, nm, em))
            mydb.commit()
            print("Student Added!")
            ...
            mycursor.execute(
                "insert into question(question,opta,optb,optc,optd,correct,tech_id)values('{}','{}','{}','{}','{}','{}',{})".format(
                    q, a, b, c, d, correct, tid))
            mydb.commit()
...
def callStudent(user_id):
    while True:
        ...
        if ch == 1:
            ...
            if per >= 33:
                status = "P"
                print("Status(P/F):", status)
            else:
                status = "F"
                print("Status(P/F):", status)

            uid1 = getUId()
            mycursor.execute("select * from user_profile where uid={}".format(uid1))
